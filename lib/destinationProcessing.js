...
exports._.tokenToMessage = function (tokenFromSourceCode) {
    let message = tokenFromSourceCode;
    if (tokenFromSourceCode.toUpperCase() === tokenFromSourceCode) {
        message = tokenFromSourceCode.split("_").join(" ");
...
exports._.tokensToAdd = function (tokensFromSourceCode, tokensStructureFromI18n) {
    ...
    return tokensFromSourceCode.filter((sourceCodeToken) => existingTokensInI18n.indexOf(sourceCodeToken) < 0);
};
...
exports._.tokensToDeactivate = function (tokensFromSourceCode, tokensStructureFromI18n) {
    let activeTokensInI18n = tokensStructureFromI18n
        ...
        .map((tokenDef) => tokenDef.name);
    return activeTokensInI18n.filter((activeTokenFromI18n) => tokensFromSourceCode.indexOf(activeTokenFromI18n) < 0);
};
...
exports._.tokensToActivate = function (tokensFromSourceCode, tokensStructureFromI18n) {
    let deactivatedTokensInI18n = tokensStructureFromI18n
        ...
        .map((tokenDef) => tokenDef.name);
    return deactivatedTokensInI18n.filter((inactiveTokenFromI18n) => {
        return tokensFromSourceCode.indexOf(inactiveTokenFromI18n) > -1;
    });
};
